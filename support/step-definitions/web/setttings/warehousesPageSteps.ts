...
import { WarehousesPage } from '../../../pageobjects/web/settings/warehousesPage';
import { randomWarehouseCode } from '../../../utilities/randomDataGenerator';
import {GetWarehouses } from '../../../rest/getEndpoints/getWarehouses';
import { DeleteOneWarehouse } from '../../../rest/deleteEndpoints/deleteOneWarehouse';
...
When(/^I remove all test warehouses$/, async () => {
    const warehouseIds = await getWarehouses.getWarehousesList();
...
When(/^I fill in the new warehouse contact info$/, async function (dataTable: DataTable) {
    ...
    if (state !== undefined && country === "United States") {
        ...
        await commonPageElements.selectTypeaheadOption(await warehousesPage.warehouseStateField, state);
    }
    if (state !== undefined && country !== "United States") {
...
Then(/^I verify that the previously saved warehouse detail info is displayed correctly$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];
    ...
    const expectedFields = [
        ...
        { field: 'warehouseCode', labelIndex: 2 },
        { field: 'warehouseModel', labelIndex: 3 }
    ];
...
Then(/^I verify that the previously saved warehouse detail info is displayed on the edit form$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];
    const fieldsToCheck = [
        { key: 'warehouseLabel', pageElement: warehousesPage.warehouseLabelField},
        { key: 'companyAssiciation', pageElement: warehousesPage.warehouseCompanyAssociationDropdown },
        { key: 'warehouseCode', pageElement: warehousesPage.warehouseCodeField },
        { key: 'warehouseModel', pageElement: warehousesPage.warehouseModelDropdown }
    ...
    for (const field of fieldsToCheck) {
        ...
        if (field.key === 'code' && value === "automationWarehouseCode") {
            value = randomWarehouseCode.toUpperCase();
        }
        if (value !== undefined) {
...
Then(/^I verify that the previously saved warehouse contact information is displayed correctly$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];

    const expectedFields = [
        { field: 'email', labelIndex: 4 },
        { field: 'phone', labelIndex: 5 },
        { field: 'street1', labelIndex: 6 },
        { field: 'street2', labelIndex: 7 },
        { field: 'country', labelIndex: 8 },
        { field: 'state', labelIndex: 9 },
        { field: 'city', labelIndex: 10 },
        { field: 'zip', labelIndex: 11 }
    ];
...
Then(/^I verify that the previously saved warehouse display preferences is displayed correctly$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];

    const expectedFields = [
        { field: 'currency', labelIndex: 12 },
        { field: 'language', labelIndex: 13 },
        { field: 'dateTime', labelIndex: 14 },
        { field: 'timezone', labelIndex: 15 },
        { field: 'measurementSystem', labelIndex: 16 }
    ];
