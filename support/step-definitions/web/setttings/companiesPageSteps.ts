...
import { CompaniesPage } from '../../../pageobjects/web/settings/companiesPage';
import { randomCompanyCode } from '../../../utilities/randomDataGenerator';
import { GetCompanies } from '../../../rest/getEndpoints/getCompanies';
import { DeleteOneCompany } from '../../../rest/deleteEndpoints/deleteOneCompany';
...
When(/^I remove all test companies$/, async () => {
    const companyIds = await getCompanies.getCompaniesList();
...
When(/^I fill in the new company contact info$/, async function (dataTable: DataTable) {
    ...
    if (state !== undefined && country === "United States") {
        ...
        await commonPageElements.selectTypeaheadOption(await companiesPage.companyStateField, state);
    }
    if (state !== undefined && country !== "United States") {
...
Then(/^I verify that the previously saved company info is displayed correctly$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];
...
Then(/^I verify that the previously saved company contact information is displayed correctly$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];
...
Then(/^I verify that the previously saved company display preferences is displayed correctly$/, async function (dataTable: DataTable) {
    const data = dataTable.hashes()[0];
    ...
    const expectedFields = [
        ...
        { field: 'measurementSystem', labelIndex: 14 }
    ];
